...
def GeneratorCNN(z, hidden_num, output_num, repeat_num, data_format, reuse):
	with nengo.Network() as net:
		...
		for idx in range(repeat_num):
			...
			stride=1, activation_fn=tf.nn.elu, data_format=data_format)
			x = tensor_layer(x, layer_func=slim.conv2d, num_outputs=hidden_num, kernel_size=3, 
			stride=1, activation_fn=tf.nn.elu, data_format=data_format)
			#if idx < repeat_num - 1:
...
def GeneratorRCNN(x,input_channel,z_num,repeat_num,hidden_num,data_format):
	with nengo.Network() as net:
		nengo_dl.configure_settings(trainable=False)
		x = tensor_layer(x, layer_func=slim.conv2d, num_outputs=hidden_num, kernel_size=3,
		stride=1, activation_fn=tf.nn.elu, normalizer_fn=slim.batch_norm,data_format=data_format)
		...
		for idx in range(repeat_num):
			...
			#if idx < repeat_num + 2:
			x=tensor_layer(x,layer_func=slim.conv2d,num_outputs=channel_num,kernel_size=3,
			stride=2,activation_fn=tf.nn.elu,normalizer_fn=slim.batch_norm,data_format=data_format)
