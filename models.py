	...
	with nengo.Network() as net:
		...
		for idx in range(repeat_num):
			...
			x=nengo_dl.tensor_layer(x,layer_func=slim.conv2d,shape_in=shape_in,num_outputs=channel_num,kernel_size=3,
				stride=1,activation_fn=None,normalizer_fn=slim.batch_norm,data_format=data_format)
			x = nengo_dl.tensor_layer(x, neuron_type, **ens_params) 
			x=tensor_layer(x,layer_func=slim.conv2d,shape_in=shape_in2,num_outputs=channel_num,kernel_size=3,
				stride=1,activation_fn=None,normalizer_fn=slim.batch_norm,data_format=data_format)
			#if idx < repeat_num + 2:
			x = nengo_dl.tensor_layer(x, neuron_type, **ens_params) 
			x=nengo_dl.tensor_layer(x,layer_func=slim.conv2d,shape_in=shape_in2,num_outputs=channel_num,kernel_size=3,
...
def DiscriminatorCNN(x,input_channel,z_num,repeat_num,hidden_num,data_format,neuron_type, ens_params):
	with nengo.Network() as net:
		...
		x=nengo_dl.tensor_layer(x,layer_func=slim.conv2d,shape_in=(1,28,28),num_outputs=hidden_num,kernel_size=3,stride=1,
		...
		for idx in range(repeat_num):
			...
			if idx < repeat_num + 2:
				x = nengo_dl.tensor_layer(x, neuron_type, **ens_params) 
				x=nengo_dl.tensor_layer(x,layer_func=slim.conv2d,shape_in=shape_in,num_outputs=channel_num,kernel_size=3,stride=2,
					activation_fn=None,data_format=data_format)
