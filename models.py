...
def DiscriminatorCNN(x,input_channel,z_num,repeat_num,hidden_num,data_format,neuron_type, ens_params):
	with nengo.Network() as net:
		...
		x=nengo_dl.tensor_layer(x,layer_func=reshape,shape=[-1, np.prod([7,7,channel_num])],data_format=data_format)
		...
		x=nengo_dl.tensor_layer(x,layer_func=reshape,h=7,w=7,c=hidden_num,data_format=data_format)
		...
		for idx in range(repeat_num):
			...
			activation_fn=None,data_format=data_format)
			#if idx < repeat_num - 1:
			x = nengo_dl.tensor_layer(x,layer_func=upscale,scale=2,data_format=data_format)
